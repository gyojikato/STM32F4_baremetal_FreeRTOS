...
static void usart_close_tx(USART_Handle_t* USART_HANDLE){
	...
	USART_HANDLE->USART_STATUS = USART_STATUS_READY;
...
{
	if(USART_HANDLE->USART_STATUS == USART_STATUS_READY){
		USART_HANDLE->TX_len = len;
		USART_HANDLE->TxBuffer = TxBuffer;
		USART_HANDLE->USART_STATUS = USART_STATUS_TX_BUSY;
...
USART_STATUS_t USART_RECEIVE_DATA_IT(USART_Handle_t* USART_HANDLE, uint8_t* RxBuffer, uint32_t len)
{
	if(USART_HANDLE->USART_STATUS == USART_STATUS_READY){
		...
		USART_HANDLE->USART_STATUS = USART_STATUS_RX_BUSY;
...
void USART_IRQ_HANDLE(USART_Handle_t* USART_HANDLE)
{
	...
	if(tempreg & USART_SR_TC_Msk){
		if(USART_HANDLE->TX_len == 0 && USART_HANDLE->USART_STATUS == USART_STATUS_TX_BUSY){
	...
	if(tempreg & USART_SR_TXE_Msk && USART_HANDLE->USART_STATUS == USART_STATUS_TX_BUSY){
	...
	tempreg = USART_HANDLE->pUSARTx->SR;
	if(tempreg & USART_SR_RXNE_Msk && USART_HANDLE->USART_STATUS == USART_STATUS_RX_BUSY){
