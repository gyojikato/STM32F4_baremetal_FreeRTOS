...
uint8_t DS1307_SET_TIME(DS1307_Handle_t* DS1307_HANDLE, uint8_t Slave_Addr){
	...
	if(DS1307_HANDLE->DS1307_COMMS_HANDLE.I2C_TRANSMIT(Slave_Addr, DS1307_ADDR_HOURS, temp_reg) == DS1307_ERROR){
		return DS1307_ERROR;
	}
	// set the value for minutes register
	if(DS1307_HANDLE->DS1307_COMMS_HANDLE.I2C_TRANSMIT(Slave_Addr, DS1307_ADDR_MINUTES, DecToBCD(DS1307_HANDLE->DS1307_TIME_HANDLE.MINUTES)) == DS1307_ERROR){
		return DS1307_ERROR;
	}
	// set the value for seconds register
	if(DS1307_HANDLE->DS1307_COMMS_HANDLE.I2C_TRANSMIT(Slave_Addr, DS1307_ADDR_SECS, DecToBCD(DS1307_HANDLE->DS1307_TIME_HANDLE.SECONDS) & ~(0x80)) == DS1307_ERROR){
...
uint8_t DS1307_SET_DATE(DS1307_Handle_t* DS1307_HANDLE, uint8_t Slave_Addr)
{
	...
	if(DS1307_HANDLE->DS1307_COMMS_HANDLE.I2C_TRANSMIT(Slave_Addr, DS1307_ADDR_DAY, DecToBCD(temp_reg & 0x07)) == DS1307_ERROR){ // clear the mask for
		return DS1307_ERROR;
	}

	// set the date
	temp_reg = DS1307_HANDLE->DS1307_DATE_HANDLE.DAY;
	if(DS1307_HANDLE->DS1307_COMMS_HANDLE.I2C_TRANSMIT(Slave_Addr, DS1307_ADDR_DATE, DecToBCD(temp_reg & 0x3F)) == DS1307_ERROR){ // clear the mask for
		return DS1307_ERROR;
	}

	// set the month
	temp_reg = DS1307_HANDLE->DS1307_DATE_HANDLE.MONTH;
	if(DS1307_HANDLE->DS1307_COMMS_HANDLE.I2C_TRANSMIT(Slave_Addr, DS1307_ADDR_MONTH, DecToBCD(temp_reg & 0x1F)) == DS1307_ERROR){ // clear the mask for
		return DS1307_ERROR;
	}

	// set the year
	temp_reg = DS1307_HANDLE->DS1307_DATE_HANDLE.YEAR;
	if(DS1307_HANDLE->DS1307_COMMS_HANDLE.I2C_TRANSMIT(Slave_Addr, DS1307_ADDR_YEAR, DecToBCD(temp_reg)) == DS1307_ERROR){ // clear the mask for
		return DS1307_ERROR;
	}
...
uint8_t DS1307_GET_TIME(DS1307_Handle_t* DS1307_HANDLE, uint8_t Slave_Addr){
	uint8_t tempreg;
	//receive hours value
	if(DS1307_HANDLE->DS1307_COMMS_HANDLE.I2C_RECEIVE(Slave_Addr, DS1307_ADDR_HOURS, &tempreg) == DS1307_ERROR){
	...
	if(DS1307_HANDLE->DS1307_COMMS_HANDLE.I2C_RECEIVE(Slave_Addr, DS1307_ADDR_MINUTES, &tempreg) == DS1307_ERROR){
		return DS1307_ERROR; // timeout error
	}
	DS1307_HANDLE->DS1307_TIME_HANDLE.MINUTES = BCDToDec(tempreg);

	// receive seconds value
	if(DS1307_HANDLE->DS1307_COMMS_HANDLE.I2C_RECEIVE(Slave_Addr, DS1307_ADDR_SECS, &tempreg) == DS1307_ERROR){
		return DS1307_ERROR; // timeout error
	}
...
uint8_t DS1307_GET_DATE(DS1307_Handle_t* DS1307_HANDLE, uint8_t Slave_Addr)
{
	...
	if(DS1307_HANDLE->DS1307_COMMS_HANDLE.I2C_RECEIVE(Slave_Addr, DS1307_ADDR_DAY, &temp_reg) == DS1307_ERROR){
		return DS1307_ERROR;
	}
	...
	// receive date
	if(DS1307_HANDLE->DS1307_COMMS_HANDLE.I2C_RECEIVE(Slave_Addr, DS1307_ADDR_DATE, &temp_reg) == DS1307_ERROR){
		return DS1307_ERROR;
	}
	DS1307_HANDLE->DS1307_DATE_HANDLE.DAY = BCDToDec(temp_reg & (0x3F)); // clear the mask for

	// receive month
	if(DS1307_HANDLE->DS1307_COMMS_HANDLE.I2C_RECEIVE(Slave_Addr, DS1307_ADDR_MONTH, &temp_reg) == DS1307_ERROR){
		return DS1307_ERROR;
	}
